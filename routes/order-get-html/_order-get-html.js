					...
					for(var i=0; i<result[0].decks.length; i++) {
						if(request.query.get_type == "view"){
							...
							"<tr data-id='"+result[0].decks[i].id+"' class='order_table_row'>"+
								...
								"<td class='import_table export_table national_table'>" + (request.query.type === 'National MX' ||  request.query.type === 'Export'?  moment(result[0].decks[i].carrierMX.date).format("DD/MM/YYYY") : moment(result[0].decks[i].carrierUS.date).format("DD/MM/YYYY")) + "</td>"+
